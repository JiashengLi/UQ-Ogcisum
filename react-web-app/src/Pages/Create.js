...
//declare global variable, record data is defaulted by blank(false) data.
let GlobalRecordData = [{"B": [false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false]}, 
...
/**
     ...
     */
function EditHeader({toneObject,toneTransport,tonePart}){
...
/**
     ...
     * @param {string}  - this function doesn't require an argument, because this page is for creation. No id for this page.
...
function Save(){
  ...
  async function postSample(){
    ...
    const PostURL = `http://wmp.interaction.courses/api/v1/?apiKey=kwjkbKI2&mode=create&endpoint=samples&sampleType=${postType}&sampleName=${postName}`
    //console.log(PostURL);
    ...
    if(postName == ""){
      ...
      //alert("Type cannot be unselected. Try again.")
    }
    else{
...
export default function CreateSample({toneObject,toneTransport,tonePart}) {
  ...
  return (
    <Template title={`Editing This Sample:`} showArrow={true}>
            <EditHeader toneObject={toneObject} toneTransport={toneTransport} tonePart={tonePart}></EditHeader>
  ...
  /**
     ...
     */
function freshBtn(type,index,Key){
  if (type === "instruments"){
